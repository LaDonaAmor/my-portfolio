...
>
  ...
  <div class="p-5">
    <div class="flex items-center justify-between gap-3">
      <p class="text-xs font-semibold uppercase tracking-[0.16em] text-accent">{impact}</p>
    ...
    <ul class="mt-5 flex flex-wrap gap-2" aria-label={`${title} technologies`}>
      {#each tech as t (t)}
        <li class="rounded-md border border-line bg-surface-2 px-2.5 py-1 text-xs text-muted">{t}</li>
      {/each}
    </ul>
