...
<script lang="ts">
	...
	function createLine(text: string, type: TerminalLine['type']): TerminalLine {
		return {
			id: nextId++,
			text,
			type
	...
	const files: Record<string, CodeLine[]> = {
		'AboutMe.ts': [
			{
				num: 1,
				text: '<span class="text-indigo-500 dark:text-indigo-400">const</span> <span class="text-blue-600 dark:text-blue-400">developer</span> = &#123;'
			},
			{
				num: 2,
				text: '  <span class="text-sky-600 dark:text-sky-400">name</span>: <span class="text-emerald-600 dark:text-emerald-400">"Racheal Ogunmodede"</span>,'
			},
			{
				num: 3,
				text: '  <span class="text-sky-600 dark:text-sky-400">role</span>: <span class="text-emerald-600 dark:text-emerald-400">"Frontend Developer"</span>,'
			},
			{
				num: 4,
				text: '  <span class="text-sky-600 dark:text-sky-400">focus</span>: <span class="text-emerald-600 dark:text-emerald-400">"Accessible UI Systems"</span>,'
			},
			{
				num: 5,
				text: '  <span class="text-sky-600 dark:text-sky-400">stack</span>: <span class="text-emerald-600 dark:text-emerald-400">"React, SvelteKit, TypeScript"</span>'
			},
			{
				num: 6,
				text: '&#125;;'
		...
		'Stack.json': [
			...
			{
				num: 2,
				text: '  <span class="text-sky-600 dark:text-sky-400">"languages"</span>: [<span class="text-emerald-600 dark:text-emerald-400">"TypeScript"</span>, <span class="text-emerald-600 dark:text-emerald-400">"JavaScript"</span>],'
			},
			{
				num: 3,
				text: '  <span class="text-sky-600 dark:text-sky-400">"frameworks"</span>: [<span class="text-emerald-600 dark:text-emerald-400">"React"</span>, <span class="text-emerald-600 dark:text-emerald-400">"Next.js"</span>, <span class="text-emerald-600 dark:text-emerald-400">"SvelteKit"</span>],'
			},
			{
				num: 4,
				text: '  <span class="text-sky-600 dark:text-sky-400">"styling"</span>: [<span class="text-emerald-600 dark:text-emerald-400">"Tailwind CSS"</span>, <span class="text-emerald-600 dark:text-emerald-400">"CSS"</span>],'
			},
			{
				num: 5,
				text: '  <span class="text-sky-600 dark:text-sky-400">"tools"</span>: [<span class="text-emerald-600 dark:text-emerald-400">"Git"</span>, <span class="text-emerald-600 dark:text-emerald-400">"Figma"</span>, <span class="text-emerald-600 dark:text-emerald-400">"Vercel"</span>]'
			},
			{
				num: 6,
				text: '&#125;'
	...
	function executeCommand() {
		...
		terminalHistory = [...terminalHistory, createLine(`➜ ~ ${commandInput}`, 'user')];
		...
		if (cmd === 'help') {
			...
			terminalHistory = [...terminalHistory, createLine('Switched to Stack.json', 'system')];
		} else if (cmd === 'clear') {
			terminalHistory = [];
		} else {
			terminalHistory = [
				...terminalHistory,
				createLine(`bash: command not found: ${cmd}. Type "help"`, 'error')
			];
		}
...
>
	<div class="relative z-10 mx-auto max-w-5xl">
		...
		<div class="mx-auto mb-12 max-w-xl text-center">
			<h2 class="font-display text-3xl font-extrabold tracking-tight text-fg md:text-4xl">
				Interactive Sandbox
			</h2>
			<p class="mt-3 text-sm text-muted">
				Explore my stack, projects, and experience through this terminal-inspired interface.
			</p>
		</div>
		...
		>
			...
			>
				<div class="flex items-center gap-1.5">
					...
					<div class="size-2.5 rounded-full bg-yellow-500/30 dark:bg-yellow-500/40"></div>
					<div class="size-2.5 rounded-full bg-green-500/30 dark:bg-green-500/40"></div>
			...
			<div class="flex border-b border-line bg-ide-tab font-mono text-xs select-none">
			...
			<div class="flex-1 bg-ide-bg p-6 font-mono text-[13px] leading-relaxed select-text">
				...
				<div class="flex min-h-5.5 animate-pulse gap-4">
					<span class="w-5 pt-0.5 text-right text-[11px] text-muted/30 select-none">
			...
			>
				<div
					class="mb-2 flex items-center gap-1.5 border-b border-line/50 pb-1 text-[10px] font-bold tracking-wider text-muted/50 uppercase select-none"
				>
					<Terminal size="11" />
					...
					{#each terminalHistory as line (line.id)}
						<p
							class={`leading-normal whitespace-pre-wrap ${
								line.type === 'system' ? 'text-indigo-600 dark:text-indigo-400' : ''
							} ${line.type === 'muted' ? 'text-muted/60' : ''} ${
								line.type === 'user' ? 'font-semibold text-fg' : ''
							} ${
								line.type === 'success' ? 'font-medium text-emerald-600 dark:text-emerald-400' : ''
							} ${line.type === 'error' ? 'font-medium text-rose-600 dark:text-rose-400' : ''}`}
						>
							{line.text}
						</p>
					{/each}
