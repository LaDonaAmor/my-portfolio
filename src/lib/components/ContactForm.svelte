<script lang="ts">
  ...
  function submit(event: SubmitEvent) {
    ...
    const mailto = `mailto:rachealogunmodede6@gmail.com`
...
<form onsubmit={submit} class="grid gap-4" novalidate>
  ...
  <label class="grid gap-2">
    ...
    <input
      ...
      required
      maxlength="100"
      class="rounded-lg border border-line bg-surface px-4 py-3 text-fg outline-none transition placeholder:text-muted/50 focus:border-accent"
  ...
  <label class="grid gap-2">
    ...
    <textarea
      ...
      maxlength="1000"
      rows="5"
      class="resize-y rounded-lg border border-line bg-surface px-4 py-3 text-fg outline-none transition placeholder:text-muted/50 focus:border-accent"
      placeholder="Tell me what you're building."
    ></textarea>
  </label>

  {#if error}
    <p role="alert" class="rounded-lg border border-red-400/30 bg-red-400/10 px-3 py-2 text-sm text-red-400">
      {error}
    </p>
  {/if}
